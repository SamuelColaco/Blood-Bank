 ...
 * orchestrates the workflow, it does not replace the lab test. The
 * crossmatchReference is a required parameter and enforced by the domain
 * (BloodComponent.allocate), not just a convention of who calls this use
 * case. See SDD Fase 3, section 3.1, and FLUXO-OPERACIONAL.md, Fluxo 5.
 */
@Injectable()
export class AllocateComponentUseCase {
  ...
  async execute(input: AllocateComponentInput): Promise<void> {
    ...
    component.allocate(input.crossmatchReference);
